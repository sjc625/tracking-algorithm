  ...
  while (current_time < end_time) {
    ...
    if (!found_aos && topo.elevation > 0.0) {
      ...
      if (start_time == current_time) {
         ...
         */
        aos_time = FindCrossingPoint(user_geo, sgp4, previous_time,
                                     current_time, true);
      }
      found_aos = true;
       ...
       */
      los_time =
          FindCrossingPoint(user_geo, sgp4, previous_time, current_time, false);
...
PassDetails_t FindNextPass(const CoordGeodetic& user_geo, SGP4* sgp4,
  ...
  DateTime start_time = DateTime::Now(true);
  DateTime previous_time(start_time);
  DateTime current_time(start_time);

  while (!found_aos || !found_los) {
    /*
     * calculate satellite position
     */
    Eci eci = sgp4->FindPosition(current_time);
    ...
    if (!found_aos && topo.elevation > 0.0) {
      ...
      std::cout << "AOS TIME: " << aos_time.ToString() << std::endl;
      found_aos = true;
    } else if (found_aos && topo.elevation < 0.0) {
      /*
       * already have the aos, but now the satellite is below the horizon,
      ...
      pd.los = los_time;
      pd.max_elevation = FindMaxElevation(user_geo, sgp4, aos_time, los_time);
      std::cout << "LOS TIME: " << los_time.ToString() << std::endl;
      break;
    } else {
      /*
       * save current time
...
//       "2 37846  54.7963 119.5777 0000994 319.0618  40.9779  1.70474628
//       6204");
//   SGP4 sgp4(tle);
